...
function gameLoop() {
    gameInterval = setInterval(show, 1000 / gameSpeed); // this value is our fps
}
...
function gameOver() {
    ...
    averageScoreElement.textContent = 
        (parseInt(averageScoreElement.textContent) * epochNumber + snake.tail.length - 1) / epochNumber;
...
class RLSnake {
    ...
    reward(state, action) {
        ...
        if (stringifiedCurrentState != stringifiedFutureState) {
            ...
            if (
                (state[this.isAheadClearIndex] == 1 && action == 0 && state[this.isAppleAheadIndex] == 1) ||
        ...
        let updateValue = this.alpha * (rewardForState + this.gamma*optimumFutureValue - this.getQ(state, action)) - 0.0001;
    ...
    implementAction(action) {
        ...
        if (snake.rotateX == 1) {
            ...
            snake.rotateY = isRight;
            snake.rotateX = 0;
        } else if (snake.rotateY == 1) {
            snake.rotateX = isRight;
            snake.rotateY = 0;
        } else if (snake.rotateY == -1) {
    ...
    getAction(state) {
        ...
        if (this.noEayLoopCount > this.maxNoEatLoopCount) {
